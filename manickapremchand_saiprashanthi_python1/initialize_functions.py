...
You can run the block many times by using the function name. 
The definition of a function specifies its name and the data the function needs to work. 
...
# User of this function will pass in N number of real numbers,
# which will be converted into a tuple.
# The for loop inside the function will iterate over the function and
# sum up all the numbers and return the sum
...
'''
An if statement allows you to set a condition on whether certain code runs. 
For example, using a variable called game_active, you can use an if statement 
to ensure the code that starts a game runs only if this variable is set to True.
'''
# Recursive function is a function that calls itself
# TO understand this, let's compute fibonacci sequence 
# where the sum of previous two numbers equals the current number. 
def fib(num):
    if(num < 2):
        return num
    else:
...
print("--------Printing a range of values using for loop-------------")
for i in range(1, 5):
    print(i)
...
For example, you can use variables such as game_active, can_edit, and polling_open, 
which take either a True or False value. When these values are True, 
